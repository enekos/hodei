# Hodei v0.2 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Take Hodei from prototype to usable keyboard-first tiling browser with integration fixes, navigation features, workspaces, and TOML configuration.
...
| File | Purpose |
|------|---------|
| `crates/hodei-core/src/db.rs` | Shared database connection + migration runner |
| `crates/hodei-core/src/history.rs` | Browser history management |
| `crates/hodei-core/src/bookmarks.rs` | Bookmark storage and search |
| `crates/hodei-core/src/suggest.rs` | Fuzzy suggestion engine for URL bar |
| `crates/hodei-core/src/workspace.rs` | Workspace manager wrapping sessions |
| `crates/hodei-core/src/config.rs` | TOML config loading and defaults |
...
| File | Changes |
|------|---------|
| `crates/hodei-core/src/lib.rs` | Add new module declarations |
| `crates/hodei-core/src/layout.rs` | `split()` accepts external ViewId |
| `crates/hodei-core/src/view.rs` | Add `next_id()` method |
| `crates/hodei-core/src/input.rs` | Add Search mode, refactor to HashMap keybindings, new commands |
| `crates/hodei-core/src/types.rs` | Add MetadataEvent type |
| `crates/hodei-core/src/session.rs` | Refactor to shared connection |
| `crates/hodei-core/src/hud.rs` | Add suggestion list, search bar |
| `crates/hodei-core/Cargo.toml` | Add `toml` dependency |
| `crates/hodei-servo/src/lib.rs` | `create_tile()` accepts ViewId, metadata channel |
| `crates/hodei-servo/src/delegate.rs` | Send metadata events via channel |
| `crates/hodei-app/src/app.rs` | Wire everything together |
| `crates/hodei-app/src/main.rs` | Updated startup flow |
...
**Files:**
- Modify: `crates/hodei-core/src/layout.rs:85-106`
- Modify: `crates/hodei-core/src/view.rs:16-34`
- Modify: `crates/hodei-servo/src/lib.rs:57-77`
- Modify: `crates/hodei-app/src/app.rs:81-93`
- Test: `crates/hodei-core/src/layout.rs` (existing tests)
- Test: `crates/hodei-core/src/view.rs` (existing tests)
...
In `crates/hodei-core/src/layout.rs`, replace lines 85-106:
...
In `crates/hodei-core/src/view.rs`, add a method to peek the next ID without creating a view:
...
In `crates/hodei-servo/src/lib.rs`, change `create_tile`:
...
pub fn create_tile(&mut self, view_id: ViewId, url_str: &str) {
    ...
    let delegate = Rc::new(HodeiWebViewDelegate::new(view_id));
...
In `crates/hodei-app/src/app.rs`, update the `SplitView` action handler:
...
```bash
git add crates/hodei-core/src/layout.rs crates/hodei-core/src/view.rs crates/hodei-servo/src/lib.rs crates/hodei-app/src/app.rs
...
- Modify: `crates/hodei-servo/src/delegate.rs`
- Modify: `crates/hodei-servo/src/lib.rs`
- Modify: `crates/hodei-app/src/app.rs`

- [ ] **Step 1: Add MetadataEvent type**

In `crates/hodei-core/src/types.rs`, add after the SessionInfo struct:
...
In `crates/hodei-servo/src/delegate.rs`:
...
In `crates/hodei-servo/src/lib.rs`, add the channel:
...
In `crates/hodei-app/src/app.rs`, add a method to process metadata:
...
```bash
git add crates/hodei-core/src/types.rs crates/hodei-servo/src/delegate.rs crates/hodei-servo/src/lib.rs crates/hodei-app/src/app.rs
...
**Files:**
- Modify: `crates/hodei-app/src/app.rs:35-50,156-168`

- [ ] **Step 1: Add JS result channel to App**

In `crates/hodei-app/src/app.rs`, add to App struct:
...
Add `dirs` as a dependency of `hodei-app` in `crates/hodei-app/Cargo.toml`:
...
In `crates/hodei-app/src/app.rs`, add to imports:
...
In `crates/hodei-core/src/view.rs`:
...
In `crates/hodei-app/src/app.rs`:
...
In `crates/hodei-core/src/view.rs` tests:
...
```bash
git add crates/hodei-core/src/view.rs crates/hodei-app/src/app.rs crates/hodei-app/Cargo.toml
...
**Files:**
- Create: `crates/hodei-core/src/db.rs`
...
In `crates/hodei-core/src/lib.rs`, add:
...
In `crates/hodei-core/src/session.rs`, change from owning to borrowing:
...
In `crates/hodei-app/src/app.rs`, update the session initialization in `resumed()`:
...
```bash
git add crates/hodei-core/src/db.rs crates/hodei-core/src/lib.rs crates/hodei-core/src/session.rs migrations/002_history_bookmarks.sql crates/hodei-app/src/app.rs
...
**Files:**
- Create: `crates/hodei-core/src/history.rs`
- Modify: `crates/hodei-core/src/lib.rs`
- Modify: `crates/hodei-app/src/app.rs`
...
In `crates/hodei-app/src/app.rs`, add HistoryManager to App:
...
```bash
git add crates/hodei-core/src/history.rs crates/hodei-core/src/lib.rs crates/hodei-app/src/app.rs
...
**Files:**
- Create: `crates/hodei-core/src/bookmarks.rs`
- Modify: `crates/hodei-core/src/lib.rs`
...
In `crates/hodei-core/src/lib.rs`:

```rust
pub mod bookmarks;
```

In `crates/hodei-core/src/input.rs`, add to the Action enum:
...
In `crates/hodei-core/src/input.rs` tests, add:
...
```bash
git add crates/hodei-core/src/bookmarks.rs crates/hodei-core/src/lib.rs crates/hodei-core/src/input.rs crates/hodei-app/src/app.rs
...
**Files:**
- Create: `crates/hodei-core/src/suggest.rs`
- Modify: `crates/hodei-core/src/lib.rs`
...
```bash
git add crates/hodei-core/src/suggest.rs crates/hodei-core/src/lib.rs
...
In `crates/hodei-core/src/hud.rs`, add methods:
...
In `crates/hodei-app/src/app.rs`, add state:
...
In `crates/hodei-core/src/input.rs`:
...
Note: Add `url` dependency to `hodei-app/Cargo.toml` for URL validation, or use the simple heuristic above (checking for `.` or `http` prefix). The simple heuristic is sufficient — no new dependency needed.
...
```bash
git add ui/hud.slint crates/hodei-core/src/hud.rs crates/hodei-core/src/input.rs crates/hodei-app/src/app.rs
...
In `crates/hodei-core/src/input.rs`, add `Search` variant to `Mode`:
...
In `crates/hodei-core/src/input.rs` tests:
...
```bash
git add crates/hodei-core/src/input.rs crates/hodei-app/src/app.rs ui/hud.slint crates/hodei-core/src/hud.rs
...
**Files:**
- Create: `crates/hodei-core/src/config.rs`
- Modify: `crates/hodei-core/Cargo.toml`
- Modify: `crates/hodei-core/src/lib.rs`

- [ ] **Step 1: Add toml dependency**

In `crates/hodei-core/Cargo.toml`, add:
...
```bash
git add crates/hodei-core/src/config.rs crates/hodei-core/src/lib.rs crates/hodei-core/Cargo.toml
git commit -m "feat: add TOML config with defaults

Config struct loaded from ~/.config/hodei/config.toml with
...
In `crates/hodei-core/src/input.rs`, add types and refactor:
...
**Files:**
- Create: `crates/hodei-core/src/workspace.rs`
...
In `crates/hodei-core/src/db.rs`, update `run_migrations`:
...
In `crates/hodei-core/src/input.rs`, add workspace actions:
...
```bash
git add crates/hodei-core/src/workspace.rs crates/hodei-core/src/lib.rs crates/hodei-core/src/input.rs crates/hodei-core/src/db.rs migrations/003_workspaces.sql
...
In `crates/hodei-app/src/app.rs`, update the App struct to hold config and workspace manager:
...
```bash
git add crates/hodei-app/src/app.rs crates/hodei-app/src/main.rs
